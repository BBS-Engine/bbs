...
public class TopDownCameraController implements ICameraController, ITickable
{
    private TopDownGameController controller;

    private Vector3d prevPosiiton;
    private Vector3d position;
    private Vector3d target = new Vector3d();

    public TopDownCameraController(TopDownGameController controller)
    {
    ...
    public void setup(Camera camera, float transition)
    {
        ...
        camera.rotation.set(MathUtils.toRad(this.controller.pitch), 0, 0);
        this.target.set(basic.prevPosition);
        this.target.lerp(basic.position, transition);
        this.target.add(cameraOffset.x, cameraOffset.y, cameraOffset.z);
