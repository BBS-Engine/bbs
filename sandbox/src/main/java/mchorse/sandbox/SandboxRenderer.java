...
public class SandboxRenderer implements IComponent
{
    ...
    public SandboxRenderer(SandboxEngine engine)
    {
        this.engine = engine;
    }
    ...
    public void renderFrameToQuality(Camera camera, Framebuffer framebuffer, int pass, boolean renderScreen, float quality)
    {
        if (quality <= 0)
        {
            quality = SandboxSettings.renderQuality.get();
        }
    ...
    private void renderScene(RenderingContext context)
    {
        ...
        if (SandboxSettings.renderTerrain.get())
        {
            ...
            if (context.isDebug() && SandboxSettings.renderTerrainDebug.get())
            {
    ...
    {
        float renderQuality = SandboxSettings.renderQuality.get();
